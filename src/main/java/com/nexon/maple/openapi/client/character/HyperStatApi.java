...
import com.nexon.maple.openapi.client.character.response.HyperStatResponse;
...
public class HyperStatApi {
    ...
    Logger logger = LoggerFactory.getLogger(HyperStatApi.class);

    public HyperStatResponse get(String ocid, String date) {
        try {
            URI uri = new URIBuilder(mapleProperties.getBase() + characterProperties.getHyperstat())
            ...
            if (response.getStatusLine().getStatusCode() == 200) {
                ...
                logger.info(body);
                HyperStatResponse res = ObjectMapperManager.camelToSnakeJsonMapper.readValue(body, HyperStatResponse.class);
                return res;
            } else {
